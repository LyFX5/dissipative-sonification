...
class Visualizer:
    def __init__(self, sim: GrayScott, audio: AudioEngine):
        ...
        self.im = self.ax.imshow(
            self.sim.V, cmap="inferno", vmin=0, vmax=1, aspect="equal"
        )
        self.ax.set_title(
            "Dissipative Structures: Reaction-Diffusion Sonification"
        )
        self.ax.axis("off")
        ...
        self.slider_f = Slider(
            ax_f, "Feed (f)", 0.01, 0.12, valinit=cfg.DEFAULT_F
        )
        self.slider_k = Slider(
            ax_k, "Kill (k)", 0.02, 0.09, valinit=cfg.DEFAULT_K
        )
    ...
    def _update_f(self, val):
        self.sim.f = val

    def _update_k(self, val):
        self.sim.k = val
